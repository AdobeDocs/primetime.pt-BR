...
<table id="table_ats_tk2_jr">  
 ...
 <tbody> 
  <tr> 
   ...
   <td>A credencial de transporte é usada para descriptografar solicitações recebidas do cliente e assinar as respostas enviadas de volta. Certifique-se de configurar o arquivo <span class="filepath"> AdobeInitial.properties</span> apropriadamente com o caminho para o arquivo de credencial de transporte, bem como a senha PKCS12 criptografada. </td> 
   <td> 
    <ul id="ul_itx_fl2_jr"> 
  ...
  <tr> 
   ...
   <td>O servidor de individualização usa a credencial da CA de individualização para assinar os certificados da máquina que emite. Certifique-se de configurar o arquivo <span class="filepath"> AdobeInitial.properties</span> apropriadamente com o caminho para o arquivo de credencial CA I15N, bem como a senha PKCS12 criptografada. </td> 
   <td> 
    <ul id="ul_xsj_nl2_jr"> 
  ...
  <tr> 
   ...
   <td> 
    ...
    <p> <code>
     ...
     <p>O License Server deve baixar automaticamente esta CRL, assim que uma solicitação de licença for processada. </p> <p importance="high">Observação: Este ponto de distribuição está <i>e não</i> marcado pelo Primetime DRM para validade. Você deve verificar se este URL é válido. Os erros resultantes de um URL inválido não serão exibidos até que os erros de validação apareçam do servidor de licenças. </p> </td> 
  </tr> 
  <tr> 
   <td> Registro </td> 
   <td>Configure <span class="filepath"> AdobeInitial.properties</span> para registro, conforme necessário. </td> 
   <td> 
    <ul id="ul_j1v_kw2_jr"> 
     ...
     <li id="li_3F46C15488114BB5B41035F710E7A19F"><span class="codeph"> log.RollSize = </span> [Roll the logs quando atingirem esse tamanho (Logs serão rolados quando o  <span class="codeph"> </span> RollIntervalor  <span class="codeph"> </span> RollSizeis for atingido, o que ocorrer primeiro)] </li> 
     <li id="li_DA32E862F7B0413885DA20633B682484"><span class="codeph"> log.ReportLogging.Enabled =</span>[ [true | false ] Especifica se um arquivo separado deve ser gerado e que contenha dados usados pelo Adobe para gerar relatórios de Individualização.] </li> 
     <li id="li_465CC6D81B8A484CBF4E7A39F7AF86AA"><span class="codeph"> log.ReportLogging.FileName = </span> [Prefixo para arquivos de log de relatório. Data/hora e <span class="filepath"> extensão .log</span> serão adicionadas ao nome do arquivo. A propriedade l<span class="codeph"> og.Level</span> não se aplica a este arquivo de log, mas <span class="codeph"> log.RollInterval</span> e <span class="codeph"> log.RollSize</span> fazem.] </li> 
    </ul> </td> 
  </tr> 
  <tr> 
   <td> Outros </td> 
   ...
   <td> 
    <ul id="ul_b3b_g1f_jr"> 
     ...
     <li id="li_5DA3C89770804B148EF6FAF01A5AD958"><span class="codeph"> keys.MinQueueSize = </span> [Procure outro lote de chaves do KGS quando houver muitas chaves restantes na fila] </li> 
  ...
  <tr> 
   ...
   <td> 
    <ul id="ul_kwq_12f_jr"> 
     ...
     <li id="li_0E34CD32CD5E47729B69B50414F93678"><span class="codeph"> log.FileName = </span> [Prefixo para arquivos de log. Data/hora e <span class="filepath"> extensão .log</span> serão adicionadas ao nome do arquivo] </li> 
     ...
     <li id="li_C2B3D111608945DA9D1428BE98D61664"><span class="codeph"> log.RollSize = </span> [Roll the logs quando atingirem esse tamanho (Logs serão rolados quando o  <span class="codeph"> </span> RollIntervalor  <span class="codeph"> </span> RollSizeis for atingido, o que ocorrer primeiro)] </li> 
    </ul> </td> 
  </tr> 
 </tbody> 
</table>
