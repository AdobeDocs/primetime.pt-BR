...
## Fase de inserção de anúncios {#section_1B18E8B5768B4873B3346294175B7340}
...
Independentemente de ocorrerem erros ou não, o TVSDK chama `AdBreakPlaybackEvent.AD_BREAK_COMPLETE` cada `AdBreakPlaybackEvent.AD_BREAK_STARTED` e `AdPlaybackEvent.AD_COMPLETED` para cada `AdPLaybackEvent.AD_STARTED`. No entanto, se os segmentos não puderem ser baixados, pode haver lacunas na linha do tempo. Quando as lacunas forem grandes o suficiente, os valores na posição do indicador de reprodução e o progresso do anúncio relatado podem exibir descontinuidades.
