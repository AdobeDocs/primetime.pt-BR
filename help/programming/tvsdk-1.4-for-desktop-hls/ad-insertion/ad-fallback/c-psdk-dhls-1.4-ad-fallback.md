...
1. Defina `fallbackOnInvalidCreative` como true para que o VMAP retorne quando o tipo de mídia para um anúncio linear/em linha for inválido para HLS.

   O valor padrão é false. Se um anúncio linear falhar porque tem um tipo de mídia inválido ou porque o anúncio não pode ser reempacotado, esse sinalizador permite que o Primetime ad decisioning siga o mesmo comportamento de fallback que se o anúncio fosse um invólucro VAST vazio.
