...
1. Definir `fallbackOnInvalidCreative` para verdadeiro, ter VMAP fallback quando o tipo de mídia de um anúncio linear/em linha for inválido para HLS.
...
No TVSDK, os únicos tipos de mídia válidos são `application/x-shockwave-flash` (VPAID) `application/x-mpegURL` (m3u8).
