...
Nesse caso, use os métodos `DRMHelper` para baixar o conteúdo do arquivo de metadados DRM, analisá-lo e verificar se a autenticação DRM é necessária.
...
1. Implemente os retornos de chamada em `DRMLoadMetadataListener`. O `loadDRMMetadata` chama esses manipuladores de evento (despacha esses eventos).
...
1. Quando o carregamento for concluído, inspecione o objeto `DRMMetadata` para verificar se a autenticação DRM é necessária.
...
1. Isso também implica a comunicação em rede, portanto, essa é também uma operação assíncrona. Use um ouvinte de evento para verificar o status da autenticação.
...
1. Se a autenticação for bem-sucedida, a reprodução do start será realizada.
