...
・ Documentação [da API TVSDK do](https://help.adobe.com/en_US/primetime/api/psdk/browser_tvsdk/index.html)navegador.

・ Guia [de programação TVSDK do](https://helpx.adobe.com/content/dam/help/en/primetime/programming-guides/psdk_browser-tvsdk.pdf)navegador.

・ [TVSDK para 1.4 DHLS para o Guia](https://helpx.adobe.com/primetime/migration-guides/tvsdk-14-dhls-browser-tvsdk-24.html)de migração do navegador TVSDK 2.4.
...
## Recursos novos e compatíveis {#what-s-new-and-supported-features}
...
・ Uma nova API, `auditudeSettings.ignoreVPAIDAds`, é adicionada para permitir que os anúncios VPAID recebidos do servidor Auditude sejam ignorados. A API não funciona para o Flash Fallback.

**Versão 2.4.11**

Os seguintes aprimoramentos e adições estão disponíveis como parte da versão 2.4.11 do Browser TVSDK:

・ O failover de segmentos ao vivo do HLS é compatível com os modos de fallback do MSE e Flash.

・ O suporte para `AuditudeSettings.creativeRepackagingDomain` API agora está disponível também para MSE. Anteriormente, era compatível apenas com o modo de fallback Flash.

・ A versão contém correções para problemas críticos do cliente. Consulte *Problemas corrigidos* em uma lista.
...
・ O TVSDK não oferece mais suporte aos Capítulos padrão ao usar o Adobe Analytics. Defina e gerencie capítulos usando seu aplicativo.
...
・ Uma nova API, AdobePSDK.otimizeFlashCalls, agora está disponível para otimizar as chamadas para melhorar a experiência de reprodução de HLS no modo de fallback Flash. Isso é desativado por padrão.

**Novo na atualização 2.4.8 (Build 6002)**

Esta atualização contém correções para problemas críticos do cliente. Consulte Problemas *corrigidos* na lista.
...
* Suporte para fluxos de trabalho de DRM e Analytics na UI Framework
...
* Substituída a `getVersion`API da `AdobePSDK.MediaPlayer` classe pela `AdobePSDK.Version` classe para informações relacionadas à versão do TVSDK. Para obter detalhes, consulte `AdobePSDK.Version` API [aqui](https://help.adobe.com/en_US/primetime/api/psdk/browser_tvsdk/AdobePSDK.Version.html).
...
>[!NOTE]
>
>As constantes preteridas enum MediaPlayer.Events e obsoletas em Enum PSDKErrorCode foram removidas na versão 2.4.6. Para obter mais informações, consulte [Conversão do navegador TVSDK 2.4.5 para a versão 2.4.6](https://helpx.adobe.com/primetime/conversion-guides/browser-tvsdk-245-to-246-for-javascript.html).

**Versão 2.4.5**

Os seguintes recursos são novos na versão 2.4.5:

* **Reproduções e anúncios completos de eventos**

   Os fluxos HLS Full Event Replay (FER) agora suportam a resolução do anúncio e os comportamentos do anúncio. Para ativar esse suporte, defina o modo de sinalização de anúncio como `MANIFEST_CUES` ao criar o `MediaPlayerItemConfig` objeto.
...
* **Suporte a conteúdo anamórfico**
   ...
   A reprodução de conteúdo anamórfico agora é compatível com o uso do MSE e da reprodução Flash.

* **Aplicação seletiva de`withCredentials`**

Quando `withCredentials` estiver definido como true, o `Access-Control-Allow-Origin` cabeçalho não poderá ser definido como um curinga. Dependendo da resposta do servidor, o TVSDK do navegador definirá seletivamente o `withCredentials` atributo. Para obter mais informações sobre esse suporte, consulte Documentos [da API TVSDK do](https://help.adobe.com/en_US/primetime/api/psdk/browser_tvsdk/index.html)navegador.
...
Para obter uma lista completa da paridade da API, consulte o Guia [de migração do](https://helpx.adobe.com/primetime/migration-guides/tvsdk-14-dhls-browser-tvsdk-24.html)TVSDK para 1.4 DHLS para o navegador TVSDK 2.4.

* **Suporte para AES de exemplo**

   Esta versão adiciona suporte para a reprodução de conteúdo criptografado AES de amostra em fallback MSE e Flash. O requisito de hospedar conteúdo AES em relação à origem segura no Google Chrome foi removido.
...
Os fluxos HLS que são entregues por meio de uma Rede de entrega de conteúdo (CDN) às vezes podem usar tokens de autenticação no manifesto e solicitações de segmento para verificação, e esses tokens podem ser fornecidos como parâmetros de URL ou como cabeçalhos de cookies. A reprodução desses fluxos agora é suportada.
...
Esta versão suporta descontinuidades causadas pela inserção de anúncios. Ele usa a tag EXT-PROGRAM-DATE-TIME seguida da tag EXT-MEDIA-SEQUENCE para sincronizar entre perfis de taxa de bits adaptáveis para uma reprodução suave.

* **Suporte a VPAID 2.0**

A definição da interface de serviço de anúncios do player de vídeo (VPAID), versão 2.0, fornece uma experiência de mídia avançada para os usuários e permite que os editores direcionem melhor os anúncios, acompanhem as impressões dos anúncios e monetizem o conteúdo de vídeo. Esta versão é compatível com anúncios VPAID lineares do JavaScript para conteúdo VOD (Video-on-demand).
...
* *Recursos de vídeo MP4 — Inserção do anúncio principal*
...
1 A tag de vídeo TVSDK do navegador não suporta streaming e DRM. O suporte a codec e contêiner não é o mesmo em todos os navegadores.

2 O Firefox assume o padrão de Flash Player para a versão 41 ou anterior.
...
**Quadro 3: Inserção do anúncio principal**

| Categoria | Tipo de conteúdo | Recurso | Flash | HTML5: FF, IE, Chrome, Android Chrome | HTML5: Safari, iOS Safari |
|--- |--- |--- |--- |--- |--- |
| Inserção do anúncio | VOD MP4 | Pré-rolo (MP4) | Não suportado | Y | Y |
| Inserção do anúncio | VOD MP4 | Pós-rolagem (MP4) | Não suportado | Y | Y |
...
**Quadro 3: Reprodução principal HLS**
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>VOD + Live</p> </td> 
   <td><p>Integração VHL do Adobe Analytics</p> </td> 
   <td><p><strong> </strong></p> <p>Y</p> </td> 
   <td><p><strong> </strong></p> <p>Y</p> </td> 
   <td><p><strong> </strong></p> <p>Y</p> </td> 
  </tr> 
...
・ Recursos de reprodução *DASH Core*

・ Recursos avançados de reprodução *DASH*

・ Recursos de proteção de conteúdo *DASH*

・ Recursos de inserção do anúncio principal *DASH*
...
・ **21465**- Error Key System Access Denied (Acesso negado ao sistema de chave de erro negado) é recebido ao reproduzir um fluxo DASH protegido por DRM depois que um fluxo DASH Live é reproduzido.
...
・ **21142**: Disponibilizando intervalos de reprodução pesquisáveis quando o player estiver em estado INICIALIZADO. Devido a essas alterações, a sessão inicial na posição agora é suportada.
...
・ **21361**: O TVSDK agora passa a ID criativa e do sistema de anúncios da resposta VAST como parâmetros de consulta na solicitação de pacote criativo para suportar regras de normalização adicionais.
...
・ **20769**: O SDK agora começa com a taxa de bits atual em busca em vez de alternar imediatamente com base na política ABR.
...
・ **13407**: Os fluxos DASH podem parar se o Firefox parar de enviar o evento &quot;ontime update&quot; durante a reprodução.

・ **16380**: Durante a reprodução de conteúdo de vídeo em áudio muxed de segmentos com horários de início não correspondentes via MSE, o erro de sincronização de áudio entre representações é acumulado em switches ABR, resultando em erro (problema de crômio nº 663686).
...
・ **19780**: A reprodução não é iniciada para conteúdo HLS e Anúncio HLS no Firefox v53 (edição n.º 354653 do Firefox).
...
・ **20179**: Flash Fallback não funciona com o Flash Player v25.0.0.171.
...
**Recursos do CSAI (Core Ad Insertion):**
...
・ **20044**: Ao usar o reempacotamento criativo, todos os anúncios no intervalo do anúncio precisam ser reempacotados com êxito, caso contrário, o intervalo do anúncio será totalmente descartado.
...
・ **14126:** A reprodução pode travar no Firefox (edição nº 1316024) devido a uma lacuna interna no buffer de origem MSE. Tente procurar para retomar a reprodução

・ **19608:** Correção para honrar o valor de deslocamento de tempo da resposta do Auditude VMAP.
...
・ **19881:** A reprodução travada e a sobreposição de buffering é exibida por tempo infinito após a busca ser executada por 3 a 4 vezes.

・ **19884:** Confirme a conformidade com os requisitos do Chrome 59 Beta Verified Media Path (VMP). O bTVSDK conseguiu reproduzir o conteúdo do DRM do Widevine com o Chrome 59 Beta.
...
・ **10075**: Ao buscar antes da linha do tempo, o evento de reprodução concluída não foi recebido no Firefox e Chrome e o evento de busca não foi recebido no Firefox.

・ **15775**: Reproduzir evento concluído não recebido no Internet Explorer do Windows 8.1.
...
・ **19222**: A chave AES-128 é solicitada uma vez para uma lista de reprodução e as solicitações subsequentes são atendidas do cache. Anteriormente, era solicitado para cada segmento.
...
・ **19605**: adRequestDomain não estava disponível no modo Flash Fallback.
...
・ **8397**: Os fluxos ao vivo do HLS gerados pelo Adobe Media Server podem não ser reproduzidos se os segmentos não iniciarem com um quadro chave.
...
・ **18093**: TimedMetadata para a tag próxima à tag assinada é retornada quando você usa a versão 24 do Flash Player no modo Flash Fallback.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>・ a applicationID no aplicativo remetente precisa ser a mesma gerada ao registrar o URL do Destinatário como um aplicativo Destinatário personalizado.</p> <p>・ O player de referência é certificado para fluxos de trabalho DASH. A estrutura da interface do usuário não está certificada.</p> <p>Para ver a lista de codecs de mídia suportados, consulte <a href="https://developers.google.com/cast/docs/media"><em>aqui</em></a>.</p> </td> 
  </tr> 
  <tr> 
   <td>FER VOD</td> 
   <td>Reprodução geral (Reproduzir, Pausar, Buscar)</td> 
  ...
  <tr> 
   ...
   <td><p>・ 20079: Regravação de buffer na busca dentro do intervalo armazenado em buffer.</p> <p>20080: O comportamento do Flash ABR é consistente com o MSE.</p> </td> 
  ...
  <tr> 
   ...
   <td><p>• 11170:</p> <p>Timed_Event não está disponível para TVSDK do navegador com MSE, diferentemente do TVSDK do navegador com Flash Fallback.</p> <p>21129: A taxa de quadros não é calculada para fluxos ao vivo.</p> </td> 
   <td> </td> 
   <td> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td>14763: Tags personalizadas que não iniciem com # não devem ser suportadas. No momento, o objeto TimedMetadata é criado e relatado para essas tags durante o Flash Fallback.</td> 
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>・ 20323: A tag PRIV ID3 usada para sinalizar o carimbo de data e hora da primeira amostra de segmento aac não é analisada pelo Safari (edição Safari #32422733)</p> <p>・ 20350: Em certos dispositivos (incluindo MAC OS X 10.1, iPad10), o Safari não fornece um evento de mudança de sinalização quando no modo de truque e, portanto, os quadros ID3 não são recebidos. (Problema no Safari #32450526)</p> </td> 
   <td> </td> 
  </tr> 
  <tr> 
   <td>VOD + Live</td> 
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>DRM</td> 
   <td> </td> 
   <td><p>12660: O player HTML5 lança um erro interno do servidor para conteúdo de hífen criptografado PlayReady expirado.</p> <p>16720: O conteúdo criptografado por DRM DASH não funcionará se o atributo Start na tag period estiver ausente.</p> <p>・ 18589: A reprodução não é suportada para fluxos de vários períodos DRM VoD protegidos por DRM com Xlink.</p> <p>・ 18653: Reprodução de Conteúdo de Vários Períodos do Widevine com Múltiplas Chaves, interrompe no primeiro período e não pode alternar para o próximo.</p> <p>・ 18656: O Playready MultiPeriod Stream, criptografado com chaves diferentes, não é reproduzido.</p> <p>O Playready 2.0 para Dash não está certificado.</p> <p> </p> <p> </p> </td> 
...
**Quadro 19: Principais recursos de inserção de anúncios (CSAI)**
...
**Quadro 20: Recursos avançados de inserção de anúncios (CSAI)**
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td> </td> 
   <td><p>・ Comportamentos de anúncios não são suportados com anúncios MP4 e conteúdo MP4.</p> <p>・ 13973: Comportamentos de anúncio personalizados - a política de SKIP não gera eventos completos quando usada com MSE.</p> <p>・ 14939: As políticas personalizadas de comportamento de anúncio ignoram e ignoram a quebra de anúncio não estão funcionando para o conteúdo DASH.</p> <p>17131: O primeiro quadro do anúncio está visível e o conteúdo é retomado no caso de política de quebra de anúncio SKIP.</p> </td> 
   <td> </td> 
   <td> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td>VPAID 2.0</td> 
   <td> </td> 
   <td><p>・ Comportamentos de anúncios não são suportados para anúncios VPAID.</p> <p>・ 15032: Anúncios VPAID em combinação com anúncios MP4 ou HLS em uma pausa de anúncio não são suportados.</p> <p>19001: No Android e no iOS, quando o anúncio VPAID é reproduzido com MP4, pois as trilhas de áudio duplas do conteúdo principal são audíveis, um dos conteúdos principais e um dos anúncios.</p> <p>・ 20762: Anúncios VPAID não são compatíveis com Picture in Picture (PIP).</p> <p>21172: O evento Play Complete não é recebido para conteúdo VOD HLS com anúncios VPAID.</p> <p>21173: onAdBreakCompleteEvent não é recebido para conteúdo VOD de HLS e anúncios VPAID de post roll.</p> </td> 
...
|---|---|---|---|---|---|
| VOD + Live | Integração VHL do Adobe Analytics |  | 19004: O rastreamento do Video Analytics não está disponível por meio da ferramenta de configuração de interface do usuário. |  |  |
