...
description: TVSDK 2.7 para Notas de versão do Android descreve as novidades ou alterações, os problemas resolvidos e conhecidos e os problemas do dispositivo no TVSDK Android 2.7
...
# TVSDK 2.7 para Notas de versão do Android {#tvsdk-for-android-release-notes}

TVSDK 2.7 para Notas de versão do Android descreve as novidades ou alterações, os problemas resolvidos e conhecidos e os problemas do dispositivo no TVSDK Android 2.7
...
O valor WebViewDebbuging está definido como Falso por padrão. Para habilitá-lo, chame setWebContentsDebuggingEnabled(true) no aplicativo.
...
* Alterações na API:
   ...
   * Uma nova API é adicionada à NetworkConfiguration::set/ getCustomUserAgent() para usar o agente de usuário personalizado.
   * Uma nova API é adicionada à NetworkConfiguration::set/ getEncodedUrlForTracking para forçar a Codificação de caracteres não seguros.
   * Uma nova API é adicionada à NetworkConfiguration::getNetworkDownCheckUrl() para definir um URL de verificação de rede no caso de failover.
...
* Alterações no SizeAvailableEvent: Anteriormente, os métodos getHeight() e getWidth() de SizeAvailableEvent em 2.5.2 eram usados para retornar a altura e a largura do quadro, retornado pelo formato de mídia. Agora, retorna a altura de saída e a largura de saída respectivamente retornadas pelo decodificador.
...
* ZD #32584 - Não é possível acessar informações completas presentes no nó &lt;Extensions> de uma resposta VAST.
   * Correção do problema na análise XML e agora NetworkAdInfo fornece as informações completas presentes no nó &lt;Extensions>.
...
* O TVSDK 2.7 é compatível com resolução simultânea de até 5 anúncios.
* No caso de resposta VMAP, as chamadas de anúncio em um único Ad break são simultâneas, e as quebras de anúncio são resolvidas sequencialmente.
...
* [Documento da API do Android C++ TVSDK](https://help.adobe.com/en_US/primetime/api/psdk/cpp/namespaces.html)  - Cada classe Java tem uma classe C++ correspondente e a documentação C++ contém mais material explicativo do que o Javadocs, portanto, consulte a documentação C++ para obter uma compreensão mais profunda da API do Java.
